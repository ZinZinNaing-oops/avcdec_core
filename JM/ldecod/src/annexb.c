  ...
  if (g_memory_buffer != NULL)
    {
        ...
        int remaining = g_memory_size - g_memory_pos;
        int to_copy = annex_b->iIOBufferSize;

        if (remaining < to_copy)
            to_copy = remaining;

        memcpy(annex_b->iobuffer,
               g_memory_buffer + g_memory_pos,
               to_copy);

        g_memory_pos += to_copy;

        annex_b->bytesinbuffer = to_copy;
...
void open_annex_b (char *fn, ANNEXB_t *annex_b)
{
  ...
  if (g_memory_buffer != NULL)
  {
    ...
    annex_b->iIOBufferSize = IOBUFFERSIZE;
    annex_b->iobuffer = (byte*)malloc(annex_b->iIOBufferSize);

    if (!annex_b->iobuffer)
      error("Memory allocation failed for memory mode", 500);
