...
private:
    ...
    UInt32 m_streamSize; //How much data decoder have(from 0 to m_streamCapacity)
    ...
    // Decodes the current buffered H.264 bitstream data
    void DecodeBuffer();

    // Checks if the internal bitstream buffer has sufficient space for new data
    bool CheckBufferSpace(UInt32 needed_bytes);

    // Signals the end of an Access Unit (AU) to the decoder
    void HandleEndOfAU();

    // Add a decoded picture buffer to the display queue
    void QueueFrameForDisplay(PictureBuffer* buffer);

    // Retrieves an available picture buffer from the free buffer pool
    PictureBuffer* GetAvailableBuffer();    

    // Processes a decoded picture from the JM decoder's internal picture list
